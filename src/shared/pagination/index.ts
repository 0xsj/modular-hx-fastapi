...
function validPart(value: unknown, max: number): value is string {
  return (
    typeof value === 'string' &&
    !/[\uD800-\uDFFF]/u.test(value) &&
    value.length > 0 &&
...
export function encode(
  ...
  return validPart(scope, 128) && validPart(position, 256)
    ? ok(
        Buffer.from(JSON.stringify([1, scope, position])).toString('base64url'),
      )
    : err(invalid());
}
export function decode(wire: string, scope: string): Result<string, Failure> {
  if (
    typeof wire !== 'string' ||
    wire.length === 0 ||
    wire.length > 1024 ||
