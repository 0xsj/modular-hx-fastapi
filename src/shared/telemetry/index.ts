...
import { failure, type Failure } from '../errors/failure.js';
...
export type TraceRef = Readonly<{ readonly __traceRef: unique symbol }>;
...
export function traceRef(
  ...
  if (!valid(traceId, 32) || !valid(spanId, 16) || typeof sampled !== 'boolean')
    return err(
...
export type Outcome =
  'success' | 'refused' | 'failed' | 'canceled' | 'timed_out';
...
export function parseOutcome(value: unknown): Result<Outcome, Failure> {
