...
/** Approximate generator timestamp, absent for versions other than 7. */
...
/** Internal encoding helper; not part of the public barrel. */
export function encodeV7(ms: number, counter: number, suffix: Uint8Array): ID {
  const stamp = ms.toString(16).padStart(12, '0');
  const sequence = (0x7000 | counter).toString(16);
  const tail = Array.from(suffix, (b, i) =>
    (i === 0 ? (b & 0x3f) | 0x80 : b).toString(16).padStart(2, '0'),
  ).join('');
