...
export class Client {
  ...
  static create(c: Config): Result<Client, Failure> {
    try {
      ...
      if (
        ...
        u.hash ||
        u.pathname !== '/' ||
        ![c.timeoutMs, c.maxRequest, c.maxResponse].every(Number.isInteger) ||
        c.timeoutMs < 1 ||
        c.timeoutMs > 30000 ||
        c.maxRequest < 1 ||
        c.maxRequest > 1048576 ||
        c.maxResponse < 1 ||
        c.maxResponse > 1048576 ||
        /[\r\n]/.test(c.bearer?.reveal() ?? '')
      )
        return err(fail('invalid', 'config'));
      return ok(new Client({ ...c }, u));
    } catch {
      return err(fail('invalid', 'config'));
    }
  }
  async do(
    ...
    if (input.trace) {
      try {
        const t = snapshot(input.trace);
        headers.traceparent =
          '00-' + t.traceId + '-' + t.spanId + '-' + (t.sampled ? '01' : '00');
      } catch {
