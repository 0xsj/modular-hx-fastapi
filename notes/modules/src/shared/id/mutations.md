...
| --- | --- |
| parse accepts other variants | `src/shared/id/id.spec.ts > id contract > I01 parse` |
| rollback resets timestamp | `src/shared/id/example.spec.ts > example: elapsed time and UUID order survive wall correction`, `src/shared/id/id.spec.ts > id contract > I04 ordering and rollback`, `src/shared/id/id.spec.ts > id contract > I05 exhaustion and recovery` |
| counter wraps on exhaustion | `src/shared/id/id.spec.ts > id contract > I05 exhaustion and recovery` |
| entropy failure commits state | `src/shared/id/id.spec.ts > id contract > I06 entropy failure atomicity` |
| counter seed loses guard bit | `src/shared/id/id.spec.ts > id contract > I04 ordering and rollback` |
| sequence repeats first value | `src/shared/id/id.spec.ts > id contract > I08 sequence` |
