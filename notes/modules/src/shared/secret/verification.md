...
Current module/root coverage: **4 named tests** (scenario groups
...
| --- | --- |
| json_discloses | src/shared/secret/secret.spec.ts > secret contract > S04 S05 S06 preserves public fields through JSON and inspection, src/shared/secret/secret.spec.ts > secret contract > S08 S09 keeps private storage and absence distinct |
| primitive_discloses | src/shared/secret/secret.spec.ts > secret contract > S01 S02 S03 S07 preserves input and redacts conversions |

TypeScript compilation additionally checks non-assignability to string with expectTypeOf. Runtime checks cover misuse and disabled custom inspection.
